...
Table~\ref{tab:primary} summarizes the two models. Both secured architectures produce zero unauthorized context exposure in the primary, held-out, and detector-targeted tests. Prompt-only produces context exposure for every unauthorized request. For the modeled path, \sbsr{}=1 follows by construction from restricting retrieval candidates to the authenticated identity's ACL; the executions confirm that the implementation preserves this invariant across the tested inputs, rather than establishing a surprising empirical effect.
...
In the original $k=2$ Smol benchmark, \arsr{} was 79.0\% prompt-only, 64.0\% pre-ACL, and 70.5\% risk-aware. Qwen's corresponding $k=2$ values were 92.5\%, 90.5\%, and 91.5\%, showing that the original spread was much smaller for the replication model. The Smol result initially resembled an authorization penalty, but the target record was present for all legitimate cases while retrieved distractors differed. In the preregistered controlled $k=1$ follow-up, all three architectures produce exactly 89.5\% on Smol and exactly 92.0\% on Qwen. Paired ACL-minus-prompt bootstrap difference is 0 with 95\% CI $[0,0]$ for both models. This equality is expected once identical target context, decoding, and model are held fixed; its value is as a confound-control check showing that the earlier Smol gap arose from retrieval composition, not the gate itself. It is not evidence that authorization is universally utility-free.

\subsection{Fixed lexical detection degrades under reformulation}
...
The white-box detector-vocabulary-targeted search reduces injection risk by 0.4745 on average (median 0.35) and reaches a final score of zero on 200/200 cases. Because it directly optimizes the known scoring functions using a detector-informed substitution set, this is evidence about the fixed lexical baseline's sensitivity, not general adaptive-attack realism. For unauthorized transformed requests, both secured architectures retain \sbsr{}=1.0 because target eligibility remains fixed by ACL state rather than wording.
