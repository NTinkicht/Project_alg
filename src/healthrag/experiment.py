...
def run_case(case: TestCase, architecture: str, records, acl, retriever, risk, llm) -> RunResult:
    ...
    if architecture == "prompt_only":
        ...
        else: retrieved = retriever.retrieve(case.prompt, allowed_aliases=acl.allowed(case.account), k=2)
        ...
        else: retrieved = retriever.retrieve(case.prompt, allowed_aliases=acl.allowed(case.account), k=1 if fuzzy.decision == "STEP_UP" else 2)
...
def main():
    ...
    rows = []; archs = ["prompt_only", "pre_retrieval_acl", "risk_aware"]
    for i, case in enumerate(cases, 1):
        for arch in archs: rows.append(run_case(case, arch, records, acl, retriever, risk, llm).to_dict())
